...
if (request.getParameter("submit") != null) {
	...
	if (validation.isValide()) {
		...
		if(!validation.existePas(Groupe.class,"nom", res)){
			...
			listeChamp.add("description");
			listeChamp.add("lien_photo");
			listeChamp.add("duree");
			listeChamp.add("nb_places");
			listeChamp.add("groupe_name");
			ArrayList<String> listeVal = new ArrayList<String>();
			listeVal.add(description);
			listeVal.add(photo);
			listeVal.add(Integer.toString(duree));
			listeVal.add(Integer.toString(nb_places));
			listeVal.add(nom_groupe);

			manager.getServRMI().updateAnim(nom, listeChamp, listeVal);
			/*for(int i = 0; i < listeHeure.getNb(); i++){
				int h = Integer.parseInt(request.getParameter("heure"+1));
				manager.getServRMI().createHorairesAnimation(nom_groupe, h);
			}*/
			validation.setMessValid("L'animation a bien été modifié");
			
			response.sendRedirect("../animation/listerAnimation.jsp");
			//return;	
		}			
	}
	...
	<table>
		<tr>
			...
			<td>
				<select name="nom" >
				<%
				ArrayList<Animation> anim = manager.getServRMI().getAllAnim();
				Iterator<Animation> itranim = anim.iterator();
			      while(itranim.hasNext()) {
			    	  Animation element = itranim.next();
			    	  if(validation.getValeurs().get("nom") != null && validation.getValeurs().get("nom").equals(element.getNom_animation())){
			        	out.print("<option selected>");
			        }else{
						out.print("<option>");
			        }
					out.println(element.getNom_animation());
					//out.println("</option>");
			      }
				%>
			...
			<td>
				...
				Iterator<Groupe> itr = groupe.iterator();
			      while(itr.hasNext()) {
			        ...
			        System.out.println(validation.getValeurs().get("groupe"));
			        System.out.println(element.getNom_groupe());
			        if(validation.getValeurs().get("groupe") != null && validation.getValeurs().get("groupe").equals(element.getNom_groupe())){
			        	out.print("<option selected>");
			        }else{
						out.print("<option>");
			        }
					out.print(element.getNom_groupe());
					out.println("</option>");
			      }
				%>
